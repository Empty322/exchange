  ...
  <div class="container">
    <div class="card" style="padding: 2rem">
      ...
      <div class="row">
        ...
        <button class="btn" @click="getPublic" v-if="isLoggedIn">
          getPublic
        </button>
        <button class="btn" @click="getPrivate" v-if="isLoggedIn">
          getPrivate
        </button>
      </div>
      {{ message }}
...
export default {
  ...
  methods: {
    ...
    async getPublic() {
      await axios
        .get("https://localhost:6001/test/public-data/")
        .then((response) => {
    ...
    async getPrivate() {
      ...
      await this.$store.dispatch("getAccessToken").then((userToken) => {
        ...
        axios
          .get("https://localhost:6001/test/private-data/")
          .then((response) => {
            console.log(response);
            this.message = response.data;
          })
